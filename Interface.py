...
class Interface_GUI(Frame):
    def __init__(self, parent, color, **kwargs):
        ...
        self.scale_img_nb = Custom_Scale.CustomScale(self, variable=self.current_img, label="Image Nb", from_=0, to=len(self.all_images) - 1, resolution=1, orient=HORIZONTAL, command=partial(self.show_image, "x",False))
    ...
    def show_image(self, event=None, reset=True,*args):
        ...
        if not self.calculated:
            image = Tiff_converter.convert_img(self.all_images[self.current_img.get()], self.normalise.get())
            image2 = image.copy()
